...
//! JPEG XL decoder integration for the [`image`] crate.
//!
...
impl<R: Read + Seek> JxlDecoder<R> {
    ...
    fn decode_into(mut self, buf: &mut [u8]) -> ImageResult<()> {
        ...
        debug_assert!(bytes_per_row.is_multiple_of(bytes_per_sample));
...
fn image_error(error: jxl::error::Error) -> ImageError {
