...
impl Tree {
    ...
    pub fn read(br: &mut BitReader, size_limit: usize) -> Result<Tree> {
        ...
        // Validate tree height and property ranges.
        //
        // This mirrors libjxl's ValidateTree(), but avoids allocating
        // `num_properties * tree.len()` entries, which can be huge.
        //
        ...
        // and store the previous range in the child frame; when returning from that child we
        // restore it. This makes memory O(num_properties + height) instead of O(num_properties *
        // tree_size).
