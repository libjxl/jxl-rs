...
impl FullModularImage {
    ...
    pub fn read_stream(
        ...
        if section_id == 1 {
            loop {
                if let Some(mut d) = self.delayed_ready_sections.try_borrow_mut() {
                    d.insert((1, grid));
                    break;
                }
            }
        ...
        if let Some(pass_to_pipeline) = pass_to_pipeline {
            ...
            loop {
                if let Some(mut r) = self.ready_transform_steps.try_borrow_mut() {
                    r.extend_from_slice(&ready_steps);
                    break;
                }
            }
            Ok(())
