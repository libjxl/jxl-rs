...
impl Frame {
    ...
    pub(crate) fn build_render_pipeline<T: RenderPipeline>(
        ...
        // For CMYK images with blending, we must defer CMS conversion until AFTER blending.
        // This is because blending in CMYK space produces different results than
        // blending in RGB space. The correct order is: blend in CMYK, then convert to RGB.
        let cmyk_needs_deferred_cms = black_channel.is_some()
        ...
        if let Some(stage) = cms_stage {
            if cmyk_needs_deferred_cms {
                // Defer CMS until after blending
        ...
        // For CMYK with deferred CMS: save reference frame in CMYK space (before CMS)
        if frame_header.can_be_referenced && !frame_header.save_before_ct {
            for i in 0..num_channels {
                pipeline = pipeline.add_save_stage(
                    &[i],
        ...
        if cmyk_needs_deferred_cms && xyb_encoded && !output_tf.is_linear() && !cms_used {
            pipeline = pipeline.add_inplace_stage(FromLinearStage::new(0, output_tf.clone()))?;
        }
