...
pub struct Frame {
    header: FrameHeader,
    toc: Toc,
    /// Absolute byte offset, from the start of the input (file-absolute;
    /// includes any ISOBMFF container), at which this frame's TOC-described
    /// section data begins (after the frame header + TOC). Set by the
    /// codestream parser; 0 until then.
    data_offset: u64,
...
impl Frame {
    ...
    /// includes any ISOBMFF container), at which this frame's TOC-described
    /// section data begins (immediately after the frame header + TOC).
    /// [`toc_entry`](Self::toc_entry) offsets are relative to this. Set by the
    /// codestream parser once the TOC is parsed.
