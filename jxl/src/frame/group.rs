...
use crate::{
    ...
    simd::{F32SimdVec, I32SimdVec, SimdDescriptor, simd_function},
...
fn adjust_quant_bias<D: SimdDescriptor>(
    ...
    let adjusted = quant - D::F32Vec::splat(d, biases[3]);
    // TODO(sboukortt): not three chained comparisons like this
    quant_i.eq(D::I32Vec::splat(d, 0)).if_then_else_f32(
        D::F32Vec::zero(d),
        quant_i.eq(D::I32Vec::splat(d, 1)).if_then_else_f32(
            D::F32Vec::splat(d, biases[c]),
            quant_i
                .eq(D::I32Vec::splat(d, -1))
                .if_then_else_f32(D::F32Vec::splat(d, -biases[c]), adjusted),
        ),
    )
}
