    ...
    pub(super) fn process_non_section(&mut self, decode_options: &JxlDecoderOptions) -> Result<()> {
        ...
        if self.decoder_state.is_none() && self.embedded_color_profile.is_none() {
            ...
            let output_color_profile = if file_header.image_metadata.xyb_encoded {
                ...
                // Apply linear TF if requested
                // TODO: Possibly remove the xyb_output_linear option completey.
                let output_encoding = if decode_options.xyb_output_linear {
                    base_encoding.with_linear_tf()
                } else {
                    base_encoding
                };

                JxlColorProfile::Simple(output_encoding)
            } else {
