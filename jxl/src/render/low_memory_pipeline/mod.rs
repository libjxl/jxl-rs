...
use crate::image::{DataTypeTag, Image, ImageDataType, KIND_GROUP, Rect};
...
impl RenderPipeline for LowMemoryRenderPipeline {
    ...
    fn get_buffer<T: ImageDataType>(&self, channel: usize) -> Result<Image<T>> {
        ...
        let expected_byte_size = (sz.0 * T::DATA_TYPE_ID.size(), sz.1);
        if let Some(b) = self.maybe_get_scratch_buffer(channel, KIND_GROUP) {
            if b.byte_size() == expected_byte_size {
