...
#[inline(always)]
fn compute_minmax<D: SimdDescriptor>(
    d: D,
    input: &[&[f32]],
    xsize: usize,
    col_min: &mut [f32],
    ...
    let in_rows: [&[f32]; 5] = [r0, r1, r2, r3, r4];
    for_each_chunk(
        d,
        col_fill_len,
        (in_rows, &mut *col_min, &mut *col_max),
        #[inline(always)]
        |_x, ([v0, v1, v2, v3, v4], mut cmin_chunk, mut cmax_chunk)| {
            let col_min_v = v0.min(v1).min(v2).min(v3).min(v4);
            let col_max_v = v0.max(v1).max(v2).max(v3).max(v4);

            cmin_chunk.write(col_min_v);
            cmax_chunk.write(col_max_v);
        },
    );
    ...
    for_each_chunk(
        ...
        #[inline(always)]
        |_x, (cmin_win, cmax_win, mut mins_chunk, mut maxs_chunk)| {
            let m0 = cmin_win.get::<0>();
            let m1 = cmin_win.get::<1>();
            let m2 = cmin_win.get::<2>();
            let m3 = cmin_win.get::<3>();
            let m4 = cmin_win.get::<4>();
            let min_v = m0.min(m1).min(m2).min(m3).min(m4);
            mins_chunk.write(min_v);

            let m0 = cmax_win.get::<0>();
            let m1 = cmax_win.get::<1>();
            let m2 = cmax_win.get::<2>();
            let m3 = cmax_win.get::<3>();
            let m4 = cmax_win.get::<4>();
            let max_v = m0.max(m1).max(m2).max(m3).max(m4);
            maxs_chunk.write(max_v);
        },
    );
}
...
fn upsample_simd<D: SimdDescriptor, const N: usize>(
    ...
    for<'b> super::row_chunks::ChunkWriter<'b, D, N>: StoreInterleaved<D, N>,
{
    ...
    for (oy, out_row) in output.iter_mut().enumerate().take(N) {
        let row_weights = &flat_kernels[oy * N..(oy + 1) * N];
        let out_scaled = ScaledChunkMut::<N>(out_row);

        for_each_chunk(
            d,
            xsize,
            (in_wins, &*mins, &*maxs, out_scaled),
            #[inline(always)]
            |_x, ([w0, w1, w2, w3, w4], minval, maxval, mut out_chunk)| {
                let mut out = [D::F32Vec::splat(d, 0.0); N];
                let wins = [w0, w1, w2, w3, w4];
                ...
                out_chunk.store_interleaved(out);
            },
        );
    }
}
