...
impl<const N: usize, const SHIFT: u8> Upsample<N, SHIFT> {
    pub fn new(ups_factors: &CustomTransformData, channel: usize) -> Self {
        ...
        for di in 0..N {
            let mut row = [[0.0f32; 25]; N];
            for dj in 0..N {
                for i in 0..5 {
                    for j in 0..5 {
                        row[dj][i * 5 + j] = kernel[di][dj][i][j];
                    }
                }
            }
            flat_kernels.push(row);
        }
...
// Two-pass SIMD upsampling matching libjxl algorithm
...
simd_function!(
    ...
    fn upsample_simd(
        ...
        // Pre-broadcast all kernel weights to SIMD vectors
        ...
        let mut col_min = [0.0f32; CHUNK_SIZE + 16];
            ...
            while cx + simd_width <= chunk_len {
                ...
                for oy in 0..n {
                    ...
                    match n {
                        ...
                        4 => D::F32Vec::store_interleaved_4(
                            results[0],
                            results[1],
                            results[2],
                            results[3],
...
impl<const N: usize, const SHIFT: u8> RenderPipelineInOutStage for Upsample<N, SHIFT> {
    ...
    fn process_row_chunk(
        ...
        // Use SIMD upsampling with interleaved stores
        upsample_simd_dispatch(input, xsize, &kernels, N, &mut output_rows[0]);
    }
}
