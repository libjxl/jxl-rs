...
simd_function!(
    ...
    fn hchroma_upsample_simd(input: &[f32], output: &mut [f32], xsize: usize) {
        ...
        // Temporary buffer for storing SIMD results before interleaving
        let mut left_buf = [0.0f32; 16];
        let mut right_buf = [0.0f32; 16];

        // SIMD loop
        let mut x = 0;
        while x + simd_width <= xsize {
            // Load: prev, cur, next (with +1 offset for border)
            ...
            left.store(&mut left_buf[..simd_width]);
            right.store(&mut right_buf[..simd_width]);

            // Interleave manually: [left0, right0, left1, right1, ...]
            for i in 0..simd_width {
                output[2 * (x + i)] = left_buf[i];
                output[2 * (x + i) + 1] = right_buf[i];
            }
        ...
        for i in x..xsize {
            ...
            let prev = input[i];
            let next = input[i + 2];
            output[2 * i] = 0.25 * prev + scaled_cur;
            output[2 * i + 1] = 0.25 * next + scaled_cur;
        }
    }
);
