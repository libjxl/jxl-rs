    ...
    ($ident:ident, $path:expr, checkpoints: $checkpoints:expr) => {
        paste::paste! {
            ...
            fn [<test_decode_test_file_ $ident>]() {
                ...
                crate::tests::decode::decode(&file, usize::MAX, false, false, None).unwrap();
            }
            ...
            fn [<test_decode_test_file_chunks_ $ident>]() {
                let path = std::path::Path::new("resources/test/").join($path);
                let file = std::fs::read(&path).unwrap();
                crate::tests::decode::decode(&file, 1, false, false, None).unwrap();
            }
            ...
            fn [<test_compare_pipelines_ $ident>]() {
                let path = std::path::Path::new("resources/test/").join($path);
                let file = std::fs::read(&path).unwrap();
                let simple_frames = crate::tests::decode::decode(&file, usize::MAX, true, false, None).unwrap().1;
                let frames = crate::tests::decode::decode(&file, usize::MAX, false, false, None).unwrap().1;
