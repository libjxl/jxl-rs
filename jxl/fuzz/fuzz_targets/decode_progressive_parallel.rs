...
fn fuzz_decode_progressive_parallel(data: &[u8]) -> Result<(), ()> {
    if data.is_empty() {
        return Ok(());
    }
    ...
    let max_threads = 2 + (rng.next_u64() as usize % 3);
    let mut runner = SimpleParallelRunner { max_threads };
            ...
            match res {
                ...
                ProcessingResult::NeedsMoreInput { mut fallback, .. } => {
                    let mut output_bufs: Vec<JxlOutputBuffer<'_>> = outputs
                        ...
                        .collect();
                    let _ = fallback.flush_pixels(&mut output_bufs, reborrow(&mut runner_opt));
...
fuzz_target!(|data: &[u8]| {
    let _ = fuzz_decode_progressive_parallel(data);
});
