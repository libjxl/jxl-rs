...
#[inline(always)]
fn idct2d_32_64_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 2048, "Data length mismatch");
    const { assert!(32usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 64 / D::F32Vec::LEN;
    let row_chunks = 32 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        for j in 0..column_chunks {
            D::F32Vec::transpose_square(d, &mut data[i * 64 + j..], column_chunks);
        }
        d.call(
            #[inline(always)]
            |_| do_idct_64_rowblock(d, &mut data[i * 64..]),
        );
    }
    for i in 0..column_chunks {
        for j in 0..row_chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 64 + i..], column_chunks);
        }
        do_idct_32(d, &mut data[i..], column_chunks);
    }
}

#[inline(always)]
fn idct2d_64_32_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 2048, "Data length mismatch");
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(32usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 32 / D::F32Vec::LEN;
    let row_chunks = 64 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        do_idct_32(d, &mut data[i..], row_chunks);
    }
    for i in 0..column_chunks {
        let tr_block = |data: &mut [<D::F32Vec as F32SimdVec>::UnderlyingArray], i, j, l| {
            D::F32Vec::transpose_square(d, &mut data[i * 64 + j + l * column_chunks..], row_chunks)
        };
        (0..2).for_each(|l| tr_block(data, i, i, l));
        for j in i + 1..column_chunks {
            (0..2).for_each(|l| tr_block(data, i, j, l));
            ...
            for l in 0..2 {
                for k in 0..D::F32Vec::LEN {
                    data.swap(
                        i * 64 + j + k * row_chunks + l * column_chunks,
                        j * 64 + i + k * row_chunks + l * column_chunks,
                    );
                }
            }
        }
        d.call(
            #[inline(always)]
            |_| do_idct_64_trh(d, &mut data[i..]),
        );
    }
}

#[inline(always)]
fn idct2d_64_64_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 4096, "Data length mismatch");
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let chunks = 64 / D::F32Vec::LEN;
    for i in 0..chunks {
        d.call(
            #[inline(always)]
            |_| do_idct_64(d, &mut data[i..], chunks),
        );
    }
    for i in 0..chunks {
        D::F32Vec::transpose_square(d, &mut data[i * 64 + i..], chunks);
        for j in i + 1..chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 64 + i..], chunks);
            D::F32Vec::transpose_square(d, &mut data[i * 64 + j..], chunks);
            for k in 0..D::F32Vec::LEN {
                data.swap(i * 64 + j + k * chunks, j * 64 + i + k * chunks);
            }
        }
        d.call(
            #[inline(always)]
...
#[inline(always)]
fn idct2d_64_128_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 8192, "Data length mismatch");
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 128 / D::F32Vec::LEN;
    let row_chunks = 64 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        for j in 0..column_chunks {
            D::F32Vec::transpose_square(d, &mut data[i * 128 + j..], column_chunks);
        }
        d.call(
            #[inline(always)]
            |_| do_idct_128_rowblock(d, &mut data[i * 128..]),
        );
    }
    for i in 0..column_chunks {
        for j in 0..row_chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 128 + i..], column_chunks);
        }
        d.call(
            #[inline(always)]
            |_| do_idct_64(d, &mut data[i..], column_chunks),
        );
    }
}

#[inline(always)]
fn idct2d_128_64_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 8192, "Data length mismatch");
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(64usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 64 / D::F32Vec::LEN;
    let row_chunks = 128 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        d.call(
            #[inline(always)]
            |_| do_idct_64(d, &mut data[i..], row_chunks),
        );
    }
    for i in 0..column_chunks {
        let tr_block = |data: &mut [<D::F32Vec as F32SimdVec>::UnderlyingArray], i, j, l| {
            D::F32Vec::transpose_square(d, &mut data[i * 128 + j + l * column_chunks..], row_chunks)
        };
        (0..2).for_each(|l| tr_block(data, i, i, l));
        for j in i + 1..column_chunks {
            (0..2).for_each(|l| tr_block(data, i, j, l));
            ...
            for l in 0..2 {
                for k in 0..D::F32Vec::LEN {
                    data.swap(
                        i * 128 + j + k * row_chunks + l * column_chunks,
                        j * 128 + i + k * row_chunks + l * column_chunks,
                    );
                }
            }
        }
        d.call(
            #[inline(always)]
            |_| do_idct_128_trh(d, &mut data[i..]),
        );
    }
}

#[inline(always)]
fn idct2d_128_128_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 16384, "Data length mismatch");
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let chunks = 128 / D::F32Vec::LEN;
    for i in 0..chunks {
        d.call(
            #[inline(always)]
            |_| do_idct_128(d, &mut data[i..], chunks),
        );
    }
    for i in 0..chunks {
        D::F32Vec::transpose_square(d, &mut data[i * 128 + i..], chunks);
        for j in i + 1..chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 128 + i..], chunks);
            D::F32Vec::transpose_square(d, &mut data[i * 128 + j..], chunks);
            for k in 0..D::F32Vec::LEN {
                data.swap(i * 128 + j + k * chunks, j * 128 + i + k * chunks);
            }
        }
        d.call(
            #[inline(always)]
...
#[inline(always)]
fn idct2d_128_256_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 32768, "Data length mismatch");
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(256usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 256 / D::F32Vec::LEN;
    let row_chunks = 128 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        for j in 0..column_chunks {
            D::F32Vec::transpose_square(d, &mut data[i * 256 + j..], column_chunks);
        }
        d.call(
            #[inline(always)]
            |_| do_idct_256_rowblock(d, &mut data[i * 256..]),
        );
    }
    for i in 0..column_chunks {
        for j in 0..row_chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 256 + i..], column_chunks);
        }
        d.call(
            #[inline(always)]
            |_| do_idct_128(d, &mut data[i..], column_chunks),
        );
    }
}

#[inline(always)]
fn idct2d_256_128_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 32768, "Data length mismatch");
    const { assert!(256usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(128usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let column_chunks = 128 / D::F32Vec::LEN;
    let row_chunks = 256 / D::F32Vec::LEN;
    for i in 0..row_chunks {
        d.call(
            #[inline(always)]
            |_| do_idct_128(d, &mut data[i..], row_chunks),
        );
    }
    for i in 0..column_chunks {
        let tr_block = |data: &mut [<D::F32Vec as F32SimdVec>::UnderlyingArray], i, j, l| {
            D::F32Vec::transpose_square(d, &mut data[i * 256 + j + l * column_chunks..], row_chunks)
        };
        (0..2).for_each(|l| tr_block(data, i, i, l));
        for j in i + 1..column_chunks {
            (0..2).for_each(|l| tr_block(data, i, j, l));
            ...
            for l in 0..2 {
                for k in 0..D::F32Vec::LEN {
                    data.swap(
                        i * 256 + j + k * row_chunks + l * column_chunks,
                        j * 256 + i + k * row_chunks + l * column_chunks,
                    );
                }
            }
        }
        d.call(
            #[inline(always)]
            |_| do_idct_256_trh(d, &mut data[i..]),
        );
    }
}

#[inline(always)]
fn idct2d_256_256_impl<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    assert_eq!(data.len(), 65536, "Data length mismatch");
    const { assert!(256usize.is_multiple_of(D::F32Vec::LEN)) };
    const { assert!(256usize.is_multiple_of(D::F32Vec::LEN)) };
    let data = D::F32Vec::make_array_slice_mut(data);
    let chunks = 256 / D::F32Vec::LEN;
    for i in 0..chunks {
        d.call(
            #[inline(always)]
            |_| do_idct_256(d, &mut data[i..], chunks),
        );
    }
    for i in 0..chunks {
        D::F32Vec::transpose_square(d, &mut data[i * 256 + i..], chunks);
        for j in i + 1..chunks {
            D::F32Vec::transpose_square(d, &mut data[j * 256 + i..], chunks);
            D::F32Vec::transpose_square(d, &mut data[i * 256 + j..], chunks);
            for k in 0..D::F32Vec::LEN {
                data.swap(i * 256 + j + k * chunks, j * 256 + i + k * chunks);
            }
        }
        d.call(
            #[inline(always)]
...
#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_32_64<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_32_64_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_64_32<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_64_32_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_64_64<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_64_64_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_64_128<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_64_128_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_128_64<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_128_64_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_128_128<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_128_128_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_128_256<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_128_256_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_256_128<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_256_128_impl(d, data)
}

#[inline(always)]
#[allow(unused_variables)]
pub fn idct2d_256_256<D: SimdDescriptor>(d: D, data: &mut [f32]) {
    idct2d_256_256_impl(d, data)
}
