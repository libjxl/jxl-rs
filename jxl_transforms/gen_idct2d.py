...
def impl_r_less_c(ROWS, COLS):
    ...
    print("}")
    maybe_wrap(
        COLS, "do_idct_%d_rowblock(d, &mut data[i * %d..])" % (COLS, COLS))
    print("}")
...
def impl_square(N):
    ...
    print("for i in 0..chunks {")
    print(
        "D::F32Vec::transpose_square(d, &mut data[i * %d + i..], chunks);" % N)
    print("for j in i+1..chunks {")
    print(
        "D::F32Vec::transpose_square(d, &mut data[j * %d + i..], chunks);" % N)
    print(
        "D::F32Vec::transpose_square(d, &mut data[i * %d + j..], chunks);" % N)
...
for ROWS, COLS in SIZES:
    ...
    print(
        "pub fn idct2d_%d_%d<D: SimdDescriptor>(d: D, data: &mut[f32]) {" % (
            ROWS, COLS)
    )
    if ROWS < 4 or COLS < 4:
