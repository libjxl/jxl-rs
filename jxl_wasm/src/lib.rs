...
use jxl::api::{JxlColorEncoding, JxlColorProfile, JxlColorType, JxlDecoderOptions, JxlPrimaries, JxlTransferFunction, JxlWhitePoint};
...
fn calculate_apng_delay(duration_ms: f64) -> Result<(u16, u16), JsValue> {
    if duration_ms < 0.0 {
        return Err(JsValue::from_str(&format!("Negative frame duration: {}", duration_ms)));
    }
    if duration_ms == 0.0 {
        return Ok((0, 1));
    }
...
fn decode_jxl_internal(jxl_data: &[u8]) -> Result<DecodeOutput, JsValue> {
    ...
    let initialized_decoder = jxl::api::JxlDecoder::<jxl::api::states::Initialized>::new(decoder_options);

    let mut decoder_with_image_info = match initialized_decoder.process(&mut input)
    ...
    loop {
        let decoder_with_frame_info = match decoder_with_image_info.process(&mut input)
        ...
        for _ in 0..extra_channels {
            outputs.push(Image::<f32>::new(frame_size)
                .map_err(|e| JsValue::from_str(&format!("Failed to create extra channel: {}", e)))?);
        }
        ...
        decoder_with_image_info = match decoder_with_frame_info.process(&mut input, &mut output_bufs)
...
fn encode_to_png(mut image_data: DecodeOutput) -> Result<Vec<u8>, JsValue> {
    ...
    match &image_data.output_profile {
        ...
        JxlColorProfile::Simple(encoding) => {
            ...
            let icc_bytes = encoding.maybe_create_profile()
    ...
    if image_data.frames.len() > 1 {
        encoder.set_animated(image_data.frames.len() as u32, image_data.num_loops)
...
pub fn get_jxl_info(jxl_data: &[u8]) -> Result<ImageInfo, JsValue> {
    ...
    let decoder_with_image_info = match initialized_decoder.process(&mut input)
        .map_err(|e| JsValue::from_str(&format!("Failed to decode header: {}", e)))? {
        ProcessingResult::Complete { result } => result,
        ProcessingResult::NeedsMoreInput { .. } => {
            return Err(JsValue::from_str("Source file truncated"));
